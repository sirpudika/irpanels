...
For the time being, the `sep` remains a private project, which is why it can only be installed via the following Github repository. 
...
As an (optional) first step, the survey is loaded into the global environment. This is not good coding practice, but it has a significant advantage. This way, you do not always have to specify the survey in the data argument later, but only the corresponding survey item, which simplifies the workflow considerably. 

It must be added that the data set used is not publicly accessible for data protection reasons. However, it can be requested [here](https://istp.ethz.ch/research/sep.html).
...
The last of the preliminary helper functions for exploratory data analysis is `eda_participants`. This function can be used to quickly determine how many participants have seen the corresponding survey item.
...
plot_groupbar_h(item = w5_q11, by = w5_q2, labels = c("Male", "Female")) +
  scale_x_discrete(labels = c("Item One", "Item Two", "Item Three", "Item Four", "Item Five"))
```
